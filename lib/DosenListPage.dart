...
import 'package:list_dosen/DetailDosenPage.dart';
import 'dart:convert';
...
class _DosenListPageState extends State<DosenListPage> {
  ...
  Future<List<ModelDosen>> fetchDosen() async {
    final response = await http.get(Uri.parse('http://192.168.43.93:8000/api/dosen'));
  ...
  Future<void> deleteDosen(String no) async {
    final response = await http.delete(Uri.parse('http://192.168.43.93:8000/api/dosen/$no'));
    if (response.statusCode == 200) {
      refreshData();
      setState(() {});
